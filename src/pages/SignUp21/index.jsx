...
const SignUp21 = () => {
  ...
  const handleSaveContinue = async (e) => {
    ...
    if (Object.values(errors).every((val) => val === "")) {
      try {
        ...
        console.log("User data stored in collection:", createDocumentResponse);
  ...
  return (
    <>
      <div className="bg-cover bg-gray-900 bg-no-repeat font-quicksand h-[900px] mx-auto p-8 sm:px-5 relative w-full" style={{ backgroundImage: "url('images/img_signin.png')" }}>
                ...
                <div className="flex flex-col justify-start w-[85%]">
                  <div className="flex flex-row items-start justify-between w-full">
                    <div className="w-[48%]">
                      ...
                      <Input name="fullName" type="text" placeholder="Enter your full name" value={formData.fullName} onChange={handleChange} className="p-0 w-full" wrapClassName="flex h-12 mt-1 w-full" shape="round" />
                    ...
                    <div className="w-[48%]">
                      ...
                      <Input name="username" type="text" placeholder="Enter username" value={formData.username} onChange={handleChange} className="p-0 w-full" wrapClassName="flex h-12 mt-1 w-full" shape="round" />
                      {errors.username && <div className="text-red-500">{errors.username}</div>}
                    </div>
                  </div>
                  <Text className="mt-4 text-gray-500 text-lg" size="txtQuicksandRegular18">Email ID *</Text>
                  <Input name="email" type="text" placeholder="Enter email" value={formData.email} onChange={handleChange} className="p-0 w-full" wrapClassName="flex h-12 mt-1 w-full" shape="round" />
                  {errors.email && <div className="text-red-500">{errors.email}</div>}
                  <Text className="mt-4 text-gray-500 text-lg" size="txtQuicksandRegular18">Password *</Text>
                  <Input name="password" type="password" placeholder="Enter password" value={formData.password} onChange={handleChange} className="p-0 w-full" wrapClassName="flex h-12 mt-1 w-full" shape="round" />
                  {errors.password && <div className="text-red-500">{errors.password}</div>}
                  <div className="flex flex-row items-start justify-between w-full">
                    <div className="w-[48%]">
                      <Text className="mt-4 text-gray-500 text-lg" size="txtQuicksandRegular18">Country *</Text>
                      <Input name="country" type="text" placeholder="Select country" value={formData.country} onChange={handleChange} className="p-0 w-full" wrapClassName="flex h-12 mt-1 w-full" shape="round" />
                    ...
                    <div className="w-[48%]">
                      ...
                      <Input name="employmentStatus" type="text" placeholder="Select status" value={formData.employmentStatus} onChange={handleChange} className="p-0 w-full" wrapClassName="flex h-12 mt-1 w-full" shape="round" />
                      {errors.employmentStatus && <div className="text-red-500">{errors.employmentStatus}</div>}
                    </div>
                  </div>
                  <Button className="common-pointer cursor-pointer font-bold leading-[normal] min-w-[169px] mt-9 mx-auto rounded-[22px] text-center text-lg" onClick={handleSaveContinue} color="gray_900" size="md" variant="fill">Save & Continue</Button>
                  <a href="javascript:" className="md:ml-[0] ml-[114px] mt-[9px] text-lg text-red-900">
                    <Text size="txtQuicksandMedium18">
                      ...
                      <span className="text-deep_orange-A200 font-quicksand text-center font-semibold">Sign In</span>
                    </Text>
                  </a>
                </div>
